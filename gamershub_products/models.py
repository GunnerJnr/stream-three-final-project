...
class GamersHubProducts(models.Model):
    ...
    def paypal_form(self):
        ...
        if settings.DEBUG:
            paypal_dict = {
            "business": settings.dev.PAYPAL_RECEIVER_EMAIL,
            "amount": self.price,
            "currency": "USD",
            "item_name": self.name,
            "invoice": "%s-%s" % (self.pk, uuid.uuid4()),
            "notify_url": settings.dev.PAYPAL_NOTIFY_URL,
            "return_url": "%s/paypal-return" % settings.dev.SITE_URL,
            "cancel_return": "%s/paypal-cancel" % settings.dev.SITE_URL
        }
        else:
            paypal_dict = {
